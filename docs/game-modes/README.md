...
`processDeadlineLock` (`src/lib/game/no-pick-handler.ts`) handles alive players who made no pick once a round's deadline passes: classic rounds past the game's second get the worst-placed unused team auto-assigned (or elimination when every round team is already used); the game's own starting round and the round after it, and turbo/cup, eliminate per their mode rules. Those two rounds are the game's own — the round it was created on and the next, not the competition's gameweeks one and two; see [classic.md](./classic.md#the-starting-round). It is idempotent and internally gated on `round.deadline <= now`, so every surface calls it unconditionally:
