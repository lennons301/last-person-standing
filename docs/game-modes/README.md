...
Computed entirely server-side in `getLivePayload` (`src/lib/game/read/live.ts`). Pure function `projectPickOutcome` lives in `src/lib/live/derive.ts`. No DB writes.
...
`processDeadlineLock` (`src/lib/game/no-pick-handler.ts`) handles alive players who made no pick once a round's deadline passes: classic no-pickers get the worst unused team auto-assigned — longest odds in the round, or worst-placed in the table where the round carries no prices (`pickWorstUnusedTeam`) — or elimination when every round team is already used; the game's own starting round and the round after it, and turbo/cup, eliminate per their mode rules. Those two rounds are the game's own — the round it was created on and the next, not the competition's gameweeks one and two; see [classic.md](./classic.md#the-starting-round). The rule itself is not in that function: `processDeadlineLock` **gathers** the rows (the round, the games on it, the competition's round sequence, each no-picker's payment-row count and the fallback team `pickWorstUnusedTeam` leaves them), hands them to the pure `decideNoPickOutcome` (`src/lib/game/no-pick-decision.ts`), and **applies** what comes back inside a transaction, so an elimination and its refund land together. Every one of the six outcomes — exempt, auto-pick, and the four eliminations — is therefore exercisable without a database, which is what the composition that eliminated paid-up survivors in #238 was not. Add a rule to that pure function, never a branch beside it.

It is idempotent and internally gated on `round.deadline <= now`, so every surface calls it unconditionally:
