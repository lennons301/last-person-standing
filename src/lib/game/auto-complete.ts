 ...
 * winner.
 *
 * The whole sequence runs in ONE transaction. A crash or statement timeout part
 * way through used to leave the game in a state nothing repairs: a payout row
 * against a game still marked `active`, or (worse) winners crowned and the pot
 * paid out with `game.status` never flipped, so the next settle pass runs the
 * completion check again and inserts the payouts a second time. The idempotency
 * guard against that re-run is the caller's `g.status !== 'active'` bail-out
 * (`checkAndMaybeCompleteOrAdvance`), which is only trustworthy if the status
 * flip lands with the payouts rather than after them.
 *
...
export async function applyAutoCompletion(
	...
	// Nothing to write and nothing to complete — bail before opening a connection.
