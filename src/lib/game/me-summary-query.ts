...
export async function getMeSummary(
	...
	const gameRows = await db
		.select({
			...
			// The round each game was played *from* — its round one. Read straight off
			// `game.starting_round_id`, which creation writes and #203 backfilled, so
			// it holds even for a game nobody has picked in yet. `current_round_id`
			// can't answer it: it advances as the game goes on.
			firstRoundNumber: round.number,
		})
		.from(gamePlayer)
		.innerJoin(game, eq(gamePlayer.gameId, game.id))
		.innerJoin(competition, eq(game.competitionId, competition.id))
		.leftJoin(round, eq(game.startingRoundId, round.id))
