...
interface PaymentsPanelProps {
	...
	/** The viewing admin's own saved pay-me handle (they're the game's creator). */
...
export function PaymentsPanel(props: PaymentsPanelProps) {
	...
	return (
		<section className="space-y-4 rounded-xl border border-border bg-card p-4 md:p-5">
			...
			<PayMeEditor
				provider={props.paymentProvider ?? null}
				handle={props.paymentHandle ?? null}
				onChange={props.onChange}
			/>
...
/**
 * Where players are pointed to pay — the admin's own Monzo/Revolut handle.
 *
