...
`standings_snapshot` records one row per (competition, team, matchday): the team's place in the official table, plus its W/D/L and points at that point. It's written by `recordStandingsSnapshot`, called from `persistLeaguePositions` (`src/lib/game/bootstrap-competitions.ts`) — the same adapter standings read that sets `team.leaguePosition`, so there's one provider request and no second cron. Don't add another writer; extend that funnel.
