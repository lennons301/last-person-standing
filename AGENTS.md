...
`/preview` renders components against hand-built fixtures — no auth, no database, no live game. It's the review surface for UI work: `/preview/game-hero` covers every `GameHero` state × game mode from descriptors shaped like `buildGameView`'s output, and `/preview/live-scores` covers the on-demand live scores pop-out from hand-built live payloads (including live-window scenarios a database won't produce on demand).
